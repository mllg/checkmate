...
double asNumber(SEXP x, const char *vname) {
    if (!isNumeric(x) || xlength(x) != 1)
        cstop(condition_error("type", "Argument '%s' must be a number", vname));
    double xd = asReal(x);
    if (ISNAN(xd))
        cstop(condition_error("missing", "Argument '%s' may not be missing", vname));
    return xd;
}

const char * asString(SEXP x, const char *vname) {
    if (!isString(x) || xlength(x) != 1)
        cstop(condition_error("type", "Argument '%s' must be a string", vname));
...
R_xlen_t asCount(SEXP x, const char *vname) {
    if (!isIntegerish(x, INTEGERISH_DEFAULT_TOL, FALSE) || xlength(x) != 1)
        cstop(condition_error("type", "Argument '%s' must be a count", vname));
    int xi = asInteger(x);
    if (xi == NA_INTEGER)
        cstop(condition_error("missing", "Argument '%s' may not be missing", vname));
    if (xi < 0)
        cstop(condition_error("value", "Argument '%s' must be >= 0", vname));
    return xi;
}

Rboolean asFlag(SEXP x, const char *vname) {
    if (!isLogical(x) || xlength(x) != 1)
        cstop(condition_error("type", "Argument '%s' must be a flag", vname));
