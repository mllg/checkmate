...
functions `assertFormula`/`assert_formula` return `x` invisibly, whereas
`checkFormula`/`check_formula` and `testFormula`/`test_formula` return
`TRUE`. If the check is not successful, `assertFormula`/`assert_formula`
throws an error message, `testFormula`/`test_formula` returns `FALSE`,
and `checkFormula`/`check_formula` return a string with the error
message. The function `expect_formula` always returns an
