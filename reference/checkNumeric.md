...
[`is.numeric`](https://rdrr.io/r/base/numeric.html). To explicitly check
for real integer or double vectors, see
...
functions `assertNumeric`/`assert_numeric` return `x` invisibly, whereas
`checkNumeric`/`check_numeric` and `testNumeric`/`test_numeric` return
`TRUE`. If the check is not successful, `assertNumeric`/`assert_numeric`
throws an error message, `testNumeric`/`test_numeric` returns `FALSE`,
and `checkNumeric`/`check_numeric` return a string with the error
message. The function `expect_numeric` always returns an
