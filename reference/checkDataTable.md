...
functions `assertDataTable`/`assert_data_table` return `x` invisibly,
whereas `checkDataTable`/`check_data_table` and
`testDataTable`/`test_data_table` return `TRUE`. If the check is not
successful, `assertDataTable`/`assert_data_table` throws an error
message, `testDataTable`/`test_data_table` returns `FALSE`, and
`checkDataTable`/`check_data_table` return a string with the error
message. The function `expect_data_table` always returns an
